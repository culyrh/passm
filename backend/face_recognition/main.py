...
import face_rocognition.main as main
import numpy as np
        ...
        if os.path.isdir(celebrity_path):
            ...
            for image_file in os.listdir(celebrity_path):
                if image_file.lower().endswith(('.jpg', '.jpeg', '.png')):
                    ...
                    # 얼굴 인식
                    image = main.load_image_file(image_path)
                    encodings = main.face_encodings(image)
...
async def recognize_face(file: UploadFile = File(...)):
    ...
    try:
        ...
        # 얼굴 위치와 인코딩 찾기
        face_locations = main.face_locations(image_np)
        face_encodings = main.face_encodings(image_np, face_locations)
        ...
        for face_encoding in face_encodings:
            ...
            matches = main.compare_faces(known_face_encodings, face_encoding, tolerance=0.6)
            distances = main.face_distance(known_face_encodings, face_encoding)
...
async def recognize_face_visual(file: UploadFile = File(...)):
    ...
    try:
        ...
        # 얼굴 위치와 랜드마크, 인코딩 찾기
        face_locations = main.face_locations(image_np)
        face_landmarks_list = main.face_landmarks(image_np)
        face_encodings = main.face_encodings(image_np, face_locations)
        ...
        for i, (face_location, face_encoding) in enumerate(zip(face_locations, face_encodings)):
            ...
            if len(known_face_encodings) > 0:
                matches = main.compare_faces(known_face_encodings, face_encoding)
                distances = main.face_distance(known_face_encodings, face_encoding)
